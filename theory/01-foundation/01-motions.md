...
**Version:** 2.0
**Purpose:** To derive motion selection from the two quantities Axiom I already names, so that a motion is a region of one space rather than an item on a list. Then to say what each region deploys, and how the names map onto the vocabulary the industry already uses.
...
* **$F_{search}$:** the buyer cannot discover, compare, or reach a viable solution at acceptable cost.
* **$F_{consensus}$:** the buyer's own stakeholders cannot reconcile competing priorities and operational risk.
* **$F_{implementation}$:** nobody can yet verify that post-signature execution will succeed without destructive disruption.
...
This is Axiom I's boundary condition. $k > k_{threshold}$ separates Turnkey deals from Structural deals, and $F_{deployed} \sim k$ requires the friction the seller deploys to scale with the specificity it manages. Both over-frictioning and under-frictioning are failures of magnitude rather than of direction.
...
Amplify each component by its own pair's gap, which is what Axiom II does:
