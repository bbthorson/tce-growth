...
**Abstract.** Packaged software rarely matches the workflow of the organization installing it. Soh, Kien and Tay-Yap named that gap *misfit*, and showed that a buyer who hits one picks from exactly four responses: change the organization to match the software, accept the shortfall, build a workaround, or customize the package. Strong and Volkoff spent three years inside a single enterprise system implementation and decomposed misfit into six domains, each running in two directions. The consequence for TCG is that the enforcement cost, implementation in field terms, belongs to neither party on its own. It is a property of the distance between a specific buyer and a specific product, which means the seller can measure it before signature and no feature list will ever reveal it.
